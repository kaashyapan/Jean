...
type BPayload =
    { BreadLink: string
      BtotalEstimatedMatches: int
      Bvalue: BSearchResult array }
...
module Encoding =
    ...
    let decode_BThumbnail: BareNET.Decoder<BThumbnail> =
        BareNET.Bare.decode_complex (fun (bwidth: int) (bheight: int) ->
            { BThumbnail.Bwidth = bwidth
              Bheight = bheight })
        |> BareNET.Bare.apply (BareNET.Bare.decode_i32)
        |> BareNET.Bare.apply (BareNET.Bare.decode_i32)

    let toBThumbnail (data: byte array) : Result<BThumbnail, string> =
        decode_BThumbnail data |> Result.map fst
    ...
    let toBSearchResult (data: byte array) : Result<BSearchResult, string> =
        decode_BSearchResult data |> Result.map fst
    ...
    let decode_BPayload: BareNET.Decoder<BPayload> =
        BareNET.Bare.decode_complex
            (fun (breadLink: string) (btotalEstimatedMatches: int) (bvalue: BSearchResult array) ->
                { BPayload.BreadLink = breadLink
                  BtotalEstimatedMatches = btotalEstimatedMatches
                  Bvalue = bvalue })
        |> BareNET.Bare.apply (BareNET.Bare.decode_string)
        |> BareNET.Bare.apply (BareNET.Bare.decode_i32)
        |> BareNET.Bare.apply (
