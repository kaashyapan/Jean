...
module Deserializers =
    ...
    let fsharplu (jsonString: string) =
        FsharpLuSerializer.deserialize<Payload> (jsonString)

    let jean (jsonString: string) = jsonString |> Payload.fromJson

    let newtonsoft (jsonString: string) =
        JsonConvert.DeserializeObject<Payload>(jsonString, new OptionConverter())
...
type JsonDeserialize() =
    let payload = Deserializers.jean Sample.sampleString
    ...
    [<Benchmark>]
    member self.NewtonsoftDeserialization() =
        Deserializers.newtonsoft Sample.sampleString

    [<Benchmark>]
    member self.FsharpLuDeserialization() =
        Deserializers.fsharplu Sample.sampleString

    [<Benchmark>]
    member self.SystemTextDeserialization() =
        Deserializers.systemText Sample.sampleString

    [<Benchmark>]
    member self.ThothNetDeserialization() = Deserializers.thoth Sample.sampleString

    [<Benchmark>]
    member self.JeanDeserialization() = Deserializers.jean Sample.sampleString
