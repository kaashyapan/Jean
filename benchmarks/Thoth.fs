...
module ThothDecoders =
    ...
    let imageDecoder =
        Decode.object (fun get -> { Image.thumbnail = get.Optional.Field "thumbnail" thumbnailDecoder })

    let videoDecoder =
        Decode.object (fun get -> { Video.thumbnail = get.Optional.Field "thumbnail" thumbnailDecoder })

    let providerDecoder =
        Decode.object (fun get -> { Provider.name = get.Required.Field "name" Decode.string })
