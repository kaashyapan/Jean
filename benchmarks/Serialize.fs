...
module Serializers =
    ...
    let systemText (payload: Payload) =
        System.Text.Json.JsonSerializer.Serialize(payload, systemTextOptions)

    let fsharplu (payload: Payload) = FsharpLuSerializer.serialize (payload)

    let jean (payload: Payload) = payload |> Payload.toJson

    let newtonsoft (payload: Payload) =
        JsonConvert.SerializeObject(payload, new OptionConverter())

    let thoth (payload: Payload) =
        payload |> ThothEncoders.toJsonPayload |> Thoth.Json.Net.Encode.toString 0
...
type JsonSerialize() =
    ...
    [<Benchmark>]
    member self.JeanSerialization() = Serializers.jean payload

    [<Benchmark>]
    member self.BareSerialization() =
        bareDto |> Bare.Msg.Encoding.ofBPayload |> Result.toOption |> Option.get
