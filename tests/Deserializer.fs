...
let tests =
    [ test "Can parse empty document" {
          ...
          let node = JsonNode.make (k)
          Expect.equal actual (k |> Seq.toArray) ""
      }
      test "Can parse document with single property" {
          let decoder (jobj: JsonNode) =
              {| firstName = Decode.Get(jobj, "firstName") |> Decode.AsString |}
      ...
      test "Can parse document with nested optional property" {
          let decoder j =
              (j </?> "person" <??> "name" <??> "first").AsStringOrNone()
      ...
      test "Can parse document with missing nested optional property" {
          let decoder j =
              (j </?> "person" <??> "name" <??> "last").AsStringOrNone()
      ...
      test "Can parse document with guid" {
          ...
          let actual =
              Json.parse decoder "{ \"id\": \"{F842213A-82FB-4EEB-AB75-7CCD18676FD5}\" }"
      ...
      test "Can convert optional int16 field to Some" {
          ...
          let actual =
              Json.parse decoder "{ \"firstName\": \"Don\", \"lastName\": \"Syme\", \"age\": 45 }"
      ...
      test "Can convert optional float field to Some" {
          ...
          let actual =
              Json.parse decoder "{ \"firstName\": \"Don\", \"lastName\": \"Syme\", \"height\": 5.8 }"
      ...
      test "Can convert optional boolean field to Some" {
          ...
          let actual =
              Json.parse decoder "{ \"firstName\": \"Don\", \"lastName\": \"Syme\", \"isMale\": \"true\" }"
      ...
      test "Can parse array of numbers" {
          let decoder j =
              Decode.AsArray(j) |> Array.map (fun x -> Decode.AsInt(x))
      ...
      test "Can convert optional obj to Some" {
          let decoder j =
              Option.get((j </?> "name").AsPropertyArrayOrNone()).Length
