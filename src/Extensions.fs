...
type Decode() =
    ...
    [<Extension>]
    static member AsString(jnode: JsonNode) : string =
        jnode.AsStringFormat CultureInfo.InvariantCulture

    [<Extension>]
    static member AsStringOrNone(jnode: JsonNode) : string option =
        jnode.TryStringFormat CultureInfo.InvariantCulture
    ...
    [<Extension>]
    static member AsBool(jnode: JsonNode) : bool =
        convertOrFail "Bool" (fun _ json -> asBool json) jnode

    [<Extension>]
    static member AsBoolOrNone(jnode: JsonNode) : bool option =
        convertOrNone (fun _ json -> asBool json) jnode
    ...
    [<Extension>]
    static member AsDateTime(jnode: JsonNode) : DateTime =
        convertOrFail "DateTime" asDateTime jnode
    ...
    [<Extension>]
    static member AsTimeSpan(jnode: JsonNode) : TimeSpan =
        convertOrFail "TimeSpan" asTimeSpan jnode
    ...
    [<Extension>]
    static member AsGuid(jnode: JsonNode) : Guid =
        convertOrFail "Guid" (fun _ json -> asGuid json) jnode

    [<Extension>]
    static member AsGuidOrNone(jnode: JsonNode) : Guid option =
        convertOrNone (fun _ json -> asGuid json) jnode
